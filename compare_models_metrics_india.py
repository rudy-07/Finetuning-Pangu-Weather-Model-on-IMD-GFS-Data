...
def main():
    parser = argparse.ArgumentParser(description="Compare two inference models and true data for multi-step autoregressive forecasts (INDIA REGION ONLY).")
    ...
    parser.add_argument("--output_txt", default="evaluation_metrics_10day_india.txt", help="Path to save output text file")
    ...
    with open(args.output_txt, 'w') as f_out:
        ...
        for step in range(1, args.steps + 1):
            ...
            log("\n" + "#"*80)
            log(f"### STEP {step} | FORECAST TARGET: {target_dt.strftime('%Y-%m-%d %H:%M:%S')} (INDIA REGION) ###")
            log("#"*80)
